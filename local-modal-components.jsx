window.DndLocalModalComponents = (() => {
    ...
    const EquipmentCompendiumModal = ({ open, items, query, category, onQueryChange, onCategoryChange, onClose, onChoose }) => {
        ...
        return <div className="fixed inset-0 z-[65] flex items-center justify-center bg-black/85 p-4" onClick={onClose}><div className="rpg-panel flex max-h-[90dvh] w-full max-w-3xl flex-col border border-amber-700 p-4" onClick={event => event.stopPropagation()}><div className="flex items-center justify-between gap-3 border-b border-gray-700 pb-3"><div><h3 className="font-fantasy text-xl font-bold uppercase tracking-wider text-amber-200">Equipo de aventurero</h3><p className="mt-1 text-xs text-gray-400">Catálogo SRD 5.1. Puedes revisar y editar cada dato antes de guardarlo.</p></div><button type="button" onClick={onClose} className="h-11 w-11 rounded border border-gray-600 text-xl text-gray-200">×</button></div><div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_12rem]"><input autoFocus value={query} onChange={event => onQueryChange(event.target.value)} placeholder="Buscar equipo" className="min-h-11 rounded border border-gray-600 bg-gray-950 px-3 text-sm text-white" /><select value={category} onChange={event => onCategoryChange(event.target.value)} className="min-h-11 rounded border border-gray-600 bg-gray-950 px-3 text-sm text-white"><option value="">Todas las categorías</option>{categories.map(item => <option key={item} value={item}>{item}</option>)}</select></div><div className="mt-3 grid flex-1 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">{matches.map(item => <article key={item.id} className="flex flex-col rounded border border-gray-700 bg-gray-900/60 p-3"><div><strong className="text-sm text-white">{item.name}</strong><span className="mt-1 block text-[10px] uppercase text-amber-300">{item.category}</span>{item.data.desc && <p className="mt-2 text-xs text-gray-400">{item.data.desc}</p>}{item.type === 'armor' && <p className="mt-2 text-xs text-cyan-200">CA {item.data.type === 'shield' ? `+${item.data.ac}` : item.data.ac} · {item.data.type}</p>}{item.type === 'weapon' && <p className="mt-2 text-xs text-red-200">{item.data.attacks?.[0]?.dmg}</p>}</div><button type="button" onClick={() => onChoose(item)} className="mt-auto min-h-10 rounded border border-amber-700 bg-amber-950/30 px-3 text-xs font-bold text-amber-100">Usar como plantilla</button></article>)}{!matches.length && <p className="py-8 text-center text-sm text-gray-500 sm:col-span-2">No hay equipo que coincida.</p>}</div></div></div>;
    };
                    ...
                    {selectedItem ? (
                        ...
                        <>
                            <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_13rem]">
                                <input autoFocus value={query} onChange={event => onQueryChange(event.target.value)} placeholder="Buscar objeto o categoría" className="min-h-11 rounded border border-gray-600 bg-gray-950 px-3 text-sm text-white outline-none focus:border-amber-500" />
