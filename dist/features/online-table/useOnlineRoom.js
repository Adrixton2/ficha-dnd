  ...
  (() => {
    ...
    function useOnlineTableController(context) {
      ...
      const saveOnlineRoomSession = room => {
        ...
        try {
          if (session) window.localStorage.setItem(ONLINE_TABLE_STORAGE_KEY, JSON.stringify({
            currentRoomCode: session.code,
            currentRoomId: session.id || session.code,
            currentRoomCollection: roomCollection(session),
            currentRoomRole: session.role,
            sharedCharacterId: session.sharedCharacterId || null,
      ...
      const attachRoomListeners = (roomSession, role) => {
        ...
        roomListenersRef.current.room = api.onSnapshot(api.doc(db, collectionName, documentId), snapshot => {
          ...
          if (nextRoom.ownerUid === firebaseUser?.uid && !roomListenersRef.current.privateEnemies && roomListenersRef.current.code === code) {
            roomListenersRef.current.playerSheets = api.onSnapshot(api.collection(db, collectionName, documentId, sheetCollectionName), sheetSnapshot => {
              setRoomPlayerSheets(sheetSnapshot.docs.map(sheet => ({
                id: sheet.id,
                ...sheet.data()
              })));
        ...
        roomListenersRef.current.members = api.onSnapshot(api.collection(db, collectionName, documentId, 'members'), snapshot => {
          setRoomMembers(snapshot.docs.map(member => ({
            id: member.id,
            ...member.data(),
            role: ['owner', 'master'].includes(member.data().role) ? 'master' : 'player'
          })).filter(member => member.active !== false).sort((a, b) => a.role === 'master' ? -1 : b.role === 'master' ? 1 : String(a.displayName).localeCompare(String(b.displayName))));
        ...
        roomListenersRef.current.participants = api.onSnapshot(api.collection(db, collectionName, documentId, 'participants'), snapshot => {
          setRoomParticipants(snapshot.docs.map(participant => ({
            id: participant.id,
            ...participant.data()
          })).sort((left, right) => Number(left.type === 'companion') - Number(right.type === 'companion')));
      ...
      const resolveRoomMembership = async (code, allowNewMember, requestedPlayerName = '', sessionHint = null) => {
        ...
        if (!session && code.length === 12) session = {
          code,
          id: code,
          collection: 'campaigns',
          schemaVersion: 2
      ...
      const activateRoomSession = (code, membership) => {
        ...
        saveOnlineRoomSession({
          ...
          playerName: membership.role === 'player' ? membership.playerName : ''
        });
      ...
      const updateParticipantInitiative = async (participant, rawValue) => {
        ...
        try {
          ...
          await api.updateDoc(api.doc(db, 'rooms', currentRoom.code, 'participants', participant.id), {
            ...
            updateSource: isCurrentRoomMaster ? 'master' : 'player'
          });
          setPreparedTurnOrder(previous => {
            ...
            const combatants = previous.map(id => {
              ...
              return combatant && id === participant.id ? {
                ...combatant,
                initiative: value
              } : combatant;
            }).filter(Boolean);
          ...
          if (participant.type === 'companion' && participant.ownerUid === firebaseUser?.uid && sharedCharacterId) {
            updateCharacterData(sharedCharacterId, previous => ({
              ...previous,
              companions: (Array.isArray(previous.companions) ? previous.companions : []).map(companion => companion.id === participant.companionId ? normalizeCompanion({
                ...companion,
                initiative: value
              }) : companion)
            }));
      ...
      const updateEnemyInitiatives = async assignments => {
        ...
        try {
          ...
          setPreparedTurnOrder(previous => {
            const combatants = previous.map(id => {
              const combatant = getCombatant(id);
              return combatant && totals.has(id) ? {
                ...combatant,
                initiative: totals.get(id)
              } : combatant;
            }).filter(Boolean);
            return combatants.length === previous.length ? orderOnlineEncounterCombatants(combatants).map(combatant => combatant.id) : previous;
          });
      ...
      const openEnemyModal = (enemy = null) => {
        ...
        setEnemyModal({
          ...
          data: enemy ? {
            ...
            dexterity: privateData?.dexterity ?? 10,
      ...
      const openBestiaryEnemyDraft = monster => {
        ...
        setBestiaryEnemyDraft({
          ...
          sourceLabel: monster.compendiumSource ? 'Compendio SRD 5.1' : 'Bestiario personal',
          name: monster.name,
          initiative: '',
          dexterity: Number.isFinite(Number(monster.srdDetails?.abilities?.dex)) ? Number(monster.srdDetails.abilities.dex) : 10,
          maxHp: monster.maxHp,
      ...
      const createEnemyFromBestiaryDraft = async () => {
        ...
        const initiativeMode = ['same', 'manual', 'none'].includes(bestiaryEnemyDraft.initiativeMode) ? bestiaryEnemyDraft.initiativeMode : 'none';
      ...
      const createSrdMonsterPrivateNotes = sourceMonster => {
        ...
        const formatGroup = (title, entries) => Array.isArray(entries) && entries.length ? `${title}:\n${entries.map(entry => `- ${entry?.name || 'Rasgo'}${entry?.desc ? `. ${entry.desc}` : ''}`).join('\n')}` : '';
        return [`Ficha de referencia: ${details.size || ''} ${details.type || 'criatura'}${details.subtype ? ` (${details.subtype})` : ''} · CR ${details.challengeRating ?? '—'}.`, details.senses ? `Sentidos: ${details.senses}.` : '', details.languages ? `Idiomas: ${details.languages}.` : '', details.resistances ? `Resistencias: ${details.resistances}.` : '', details.immunities ? `Inmunidades: ${details.immunities}.` : '', details.conditionImmunities ? `Inmunidades de condición: ${details.conditionImmunities}.` : '', formatGroup('Rasgos', details.traits), formatGroup('Acciones', details.actions), formatGroup('Acciones adicionales', details.bonusActions), formatGroup('Reacciones', details.reactions), formatGroup('Acciones legendarias', details.legendaryActions)].filter(Boolean).join('\n\n');
      };
      ...
      const addSrdMonsterToBestiary = sourceMonster => {
        ...
        try {
          ...
          setBestiaryNotice(error?.name === 'QuotaExceededError' ? 'No hay espacio local suficiente para guardar la plantilla.' : 'No se pudo añadir la criatura al Bestiario.');
        }
      };
      ...
      const saveEnemy = async () => {
        ...
        if (!name || initiative !== null && !Number.isFinite(initiative) || !Number.isFinite(maxHp) || !Number.isFinite(currentHp) || !Number.isFinite(tempHp) || maxHp < 0 || currentHp < 0 || currentHp > maxHp || tempHp < 0) {
          setOnlineTableError('Revisa nombre, Destreza, iniciativa y valores de vida del enemigo.');
          return;
        }
      ...
      const removeCombatantFromTurnOrder = async ({
        roomCode,
        combatantId,
        additionalCombatantIds = [],
        reason = 'removed',
        removeEnemyDocuments = false,
        removePlayerUid = null
      }) => {
        const {
          db,
          api
      ...
      const saveOnlineCondition = async () => {
        ...
        if (target.type === 'enemy') {
          ...
          if (target.ownerUid === firebaseUser?.uid && target.characterId === sharedCharacterId) {
            if (target.type === 'companion') updateCharacterData(sharedCharacterId, previous => ({
              ...previous,
              companions: (previous.companions || []).map(companion => companion.id === target.companionId ? normalizeCompanion({
                ...companion,
                conditions: next
              }) : companion)
      ...
      const openEffectModal = (effect = null, preferredTarget = null) => {
        ...
        setEffectModal({
          ...
          data: effect ? {
            ...effect,
            concentration: Boolean(effect.concentration || effect.requiresConcentration)
          } : {
            name: '',
            targetId: defaultTarget?.id || (isCurrentRoomMaster ? 'global' : ''),
            targetType: defaultTarget?.type === 'enemy' ? 'enemy' : defaultTarget ? 'player' : isCurrentRoomMaster ? 'global' : 'player',
            durationType: 'rounds',
            remaining: 1,
            maximum: 1,
      ...
      const changeOnlineTableVisibility = shouldOpen => {
        ...
        const commitVisibility = () => {
          if (typeof ReactDOM.flushSync === 'function') ReactDOM.flushSync(() => setOnlineTableOpen(shouldOpen));else setOnlineTableOpen(shouldOpen);
        };
      ...
      const finishOnlineTableDockDrag = event => {
        ...
        if (drag.moved && drag.lastPosition) {
          try {
            window.localStorage.setItem('dnd_online_table_dock_position_v1', JSON.stringify(drag.lastPosition));
          } catch (error) {}
        }
        drag.pointerId = null;
      ...
      const leaveOnlineRoom = async () => {
        ...
        try {
          ...
          if (currentRoom.collection === 'campaigns') {
            ...
            const batch = api.writeBatch(db);
            batch.update(api.doc(db, 'campaigns', currentRoom.id, 'members', uid), {
              active: false,
              lastSeen: api.serverTimestamp(),
              updatedAt: api.serverTimestamp()
            });
      ...
      const kickRoomPlayer = async member => {
        ...
        try {
          ...
          const outcome = await removeCombatantFromTurnOrder({
            ...
            additionalCombatantIds: companionIds,
            reason: 'kicked',
            removePlayerUid: member.uid
          });
          if (currentRoom.collection === 'campaigns') {
            ...
            await api.updateDoc(activeRoomDoc(api, db, 'members', member.uid), {
              active: false,
              blocked: true,
              updatedAt: api.serverTimestamp()
            });
      ...
      const closeOnlineRoom = async () => {
        ...
        try {
          ...
          await api.updateDoc(activeRoomDoc(api, db), {
            status: 'closed',
            joinEnabled: false,
            updatedAt: api.serverTimestamp()
          });
      ...
      const copyTextToClipboard = async value => {
        ...
        try {
          ...
          console.warn('[Mesa] Copia compatible no disponible.', error);
        }
        textarea.remove();
        return copied;
      };
        ...
        if (navigator.share) {
          try {
            await navigator.share({
              ...
              text: `Únete a la sala ${roomInvite.code}`,
              url: roomInvite.url
            });
            setRoomInvite({
              isOpen: false,
      ...
      const loadCloudCampaigns = async () => {
        ...
        try {
          ...
          const active = references.docs.map(item => ({
            id: item.id,
            ...item.data()
          })).filter(item => item.active !== false).sort((left, right) => Number(right.updatedAt?.toMillis?.() || 0) - Number(left.updatedAt?.toMillis?.() || 0));
          const sessions = [];
