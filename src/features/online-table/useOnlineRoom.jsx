        ...
        const {
            ...
            const buildPreparedTurnOrder = () => {
                ...
                setPreparedTurnOrder(previous => {
                    ...
                    const selectedIds = retained.length
                        ? retained
                        : publicCombatants.map(participant => participant.id).filter(id => availableIds.has(id));
            ...
            const togglePreparedParticipant = (id) => {
                ...
                setPreparedTurnOrder(previous => previous.includes(id)
                    ? previous.filter(participantId => participantId !== id)
                    : [...previous, id]);
            };
            ...
            const createOnlineRoom = async (requestedName = '') => {
                try {
                    ...
                    await batch.commit();
                    const session = { code, id: code, collection: 'campaigns', schemaVersion: 2, role: 'master', playerName: '', name: campaignName };
            ...
            useEffect(() => {
                ...
                const syncPresence = () => {
                    if (document.visibilityState === 'hidden') updateCurrentPresence(true, false);
                    else markPresent();
                };
                syncPresence();
