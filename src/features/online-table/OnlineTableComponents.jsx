        ...
        <section className="online-party-hub" aria-labelledby="online-party-title">
            ...
            <div className="online-party-grid">
                {playerMembers.map(member => {
                    ...
                    return <article key={member.id} className={`online-party-card ${presenceStatus === 'away' ? 'is-away' : connected ? '' : 'is-offline'}`}>
                        <div className="online-party-card__identity">
                            ...
                            <div><strong>{participant?.name || 'Personaje sin compartir'}</strong><span>{snapshot?.identity?.className || participant?.className || 'Sin clase'} · Nivel {snapshot?.identity?.level || participant?.level || '—'}</span><em>Jugador: {member.displayName || 'Sin identificar'}</em><small className="online-party-card__sync">{presenceLabel} · {syncLabel}</small></div>
                            <i className={presenceStatus === 'online' ? 'is-connected' : presenceStatus === 'away' ? 'is-away' : ''} title={presenceLabel} />
                        </div>
                        {participant ? <>
...
const OnlineGroupRoster = ({ participants = [], members = [], compact = false, onOpenGroup }) => {
    ...
    return <section className={`online-group-roster ${compact ? 'is-compact' : ''}`}>
        ...
        <div className="online-group-roster__grid">{visibleMembers.map(member => {
            ...
            const hpPercent = hp.maxHp > 0 ? Math.min(100, hp.currentHp / hp.maxHp * 100) : 0;
            const conditions = window.DndOnlineTableUtils.normalizeOnlineConditions(participant?.conditions);
            return <article key={member.id} className={connected ? 'is-online' : presenceStatus === 'away' ? 'is-away' : 'is-offline'}>
                <div className="online-group-roster__identity">{participant ? <OnlineCombatantAvatar combatant={participant} className="h-12 w-12 text-sm"/> : <span className="online-group-roster__empty">?</span>}<div><small>{presenceLabel}</small><strong>{participant?.name || member.displayName || 'Jugador'}</strong><p>{participant ? `${participant.className || 'Sin clase'} · Nivel ${participant.level || '—'}` : `Jugador: ${member.displayName || 'Sin identificar'}`}</p></div><i/></div>
...
const OnlineCampaignLobby = ({ roomData, isMaster, members = [], participants = [], sheets = [], enemies = [], ownParticipant, onSelect, onInvite, onShareCharacter }) => {
    const playerMembers = members.filter(member => member.role !== 'master');
...
const OnlineTacticalDetailPanel = ({ selected, isEnemy, privateData, hp, hpPercent = 0, canSeeHp, canEdit, conditions = [], effects = [], currentUid, onAvatarPreview, onEditEnemy, onDeleteEnemy, onOpenHealth, onQuickHp, onDefeat, onAddCondition, onRemoveCondition, onAddEffect, onAdjustEffect, onFinishEffect, canManageEffect }) => {
    ...
    React.useEffect(() => {
        ...
        const closeOnEscape = event => { if (event.key === 'Escape') setReferenceOpen(false); };
    ...
    return <><aside className={`tactical-detail-panel online-tactical-detail ${isEnemy ? 'is-enemy' : isCompanion ? 'is-companion' : 'is-player'} ${hpTone}`} aria-label={`Detalle de ${selected.name || 'combatiente'}`}>
        ...
        <header className="online-tactical-detail__hero">
            ...
            <div><small>{typeLabel}</small><h4>{selected.name || 'Combatiente'}</h4><p><span>Iniciativa {selected.initiative ?? '—'}</span><span>{isEnemy ? selected.visibleState || 'Estado oculto' : selected.presenceLabel || 'Desconectado'}</span></p></div>
            {isEnemy && canEdit && <div className="online-tactical-detail__enemy-actions">{hasPrivateReference && <button type="button" onClick={() => setReferenceOpen(true)} className="is-reference">Ver ficha</button>}<button type="button" onClick={onEditEnemy}>Editar</button><button type="button" onClick={onDeleteEnemy} className="is-danger">Eliminar</button></div>}
        </header>
    ...
    </aside>{referenceOpen && hasPrivateReference && ReactDOM.createPortal(<div className="online-combat-modal-overlay is-enemy-reference" onMouseDown={event => { if (event.target === event.currentTarget) setReferenceOpen(false); }}><article className="online-combat-modal enemy-reference-modal" role="dialog" aria-modal="true" aria-labelledby="enemy-reference-title"><header className="online-combat-modal__header"><span aria-hidden="true">◈</span><div><small>Consulta privada del Máster</small><h3 id="enemy-reference-title">{selected.name || 'Ficha del enemigo'}</h3><p>Información de referencia fuera del panel operativo del combate.</p></div><button type="button" onClick={() => setReferenceOpen(false)} aria-label="Cerrar ficha">×</button></header><div className="online-combat-modal__body enemy-reference-modal__body"><section className="enemy-reference-modal__stats"><span><small>PV máximos</small><strong>{privateData?.maxHp ?? '—'}</strong></span><span><small>Clase de armadura</small><strong>{privateData?.armorClass ?? '—'}</strong></span><span><small>Iniciativa</small><strong>{selected.initiative ?? '—'}</strong></span></section><section className="enemy-reference-modal__notes"><header><small>Ficha y notas privadas</small><strong>Datos del monstruo</strong></header><pre>{privateData.notes}</pre></section></div><footer className="online-combat-modal__footer"><p><span aria-hidden="true">◈</span> Esta información solo es visible para el Máster.</p><button type="button" onClick={() => setReferenceOpen(false)}>Cerrar</button><button type="button" className="is-primary" onClick={() => { setReferenceOpen(false); onEditEnemy?.(); }}>Editar enemigo</button></footer></article></div>, document.body)}</>;
};
