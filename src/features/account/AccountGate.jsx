            ...
            {panelOpen && ReactDOM.createPortal(<div className="account-dialog-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) setPanelOpen(false); }}>
                <section className="account-dialog" role="dialog" aria-modal="true" aria-labelledby="account-dialog-title">
                    ...
                    <div className="account-dialog__body">
                        ...
                        {user.isAnonymous ? <button type="button" className="account-protect-action" disabled={!!busy} onClick={() => run('link', () => window.firebaseAccount.linkAnonymousWithGoogle())}><span>G</span><div><strong>Proteger y sincronizar</strong><small>Conservarás el mismo acceso y tus personajes.</small></div></button> : <section className="account-alias"><label><span>Alias visible</span><small>Opcional. El email no se guarda en Firestore.</small><input value={alias} maxLength="80" onChange={event => setAlias(event.target.value)} placeholder="Nombre de jugador" /></label><button type="button" disabled={busy === 'alias'} onClick={saveAlias}>{busy === 'alias' ? 'Guardando…' : 'Guardar alias'}</button></section>}
