        ...
        function KaelCharacterSheet() {
            ...
            const presenceAwareRoomParticipants = useMemo(() => roomParticipants.map(participant => {
                ...
                const presence = currentRoom?.collection === 'campaigns'
                    ? window.DndOnlineTableUtils.getOnlinePresence(member, participant, onlinePresenceNow)
                    : { status: participant.connected === false ? 'offline' : 'online', label: participant.connected === false ? 'Desconectado' : 'Conectado', lastSeenAt: 0 };
                return { ...participant, presenceStatus: presence.status, presenceLabel: presence.label, presenceLastSeenAt: presence.lastSeenAt };
            ...
            const encounterEnemies = (roomData?.status === 'active' || roomData?.status === 'paused')
                ? publicCombatants.filter(combatant => roomData.turnOrder?.includes(combatant.id))
                : publicCombatants;
            ...
            const getCombatant = (id) => encounterCombatants.find(combatant => combatant.id === id || combatant.ownerUid === id)
                ...
                || publicCombatants.find(combatant => combatant.id === id || combatant.ownerUid === id)
                || null;
